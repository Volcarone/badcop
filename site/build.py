...
TEMPLATES = [
    ...
    {"slug": "second-payment-reminder-email", "step": "firm", "offset": 7,
     "title": "Second payment reminder email (after the first was ignored)",
     "h1": "Second reminder: what to send when the first one got no reply",
     "when": "About a week after the first reminder went unanswered, or seven days past due, whichever is later.",
     "why": "A second reminder that repeats the first teaches the client that reminders are noise. This one changes register: it asks for a date, gives a window, and mentions the terms. The escalation is the message.",
     "subject": "Second reminder: invoice {invoice_id}, {currency} {amount} outstanding",
     ...
     "tips": ["Reference the first reminder explicitly. It shows there is a record.", "Ask for a date, not for payment; a date is easier to commit to and makes the next email straightforward if it passes.", "Keep the same sender address as the first reminder so the thread stays intact."]},
    {"slug": "polite-payment-reminder-email", "step": "friendly", "offset": 1,
     "title": "Polite payment reminder email that still gets results",
     "h1": "A polite payment reminder that does not sound like an apology",
     "when": "Any time in the first week after the due date. Polite is fine; apologetic is not.",
     "why": "Most 'polite' reminders fail because they apologise for existing ('sorry to bother you'). This one is courteous and completely clear about the amount, the date and what you need. Politeness is in the tone, not in the vagueness.",
     "subject": "Payment reminder: invoice {invoice_id} ({currency} {amount})",
     "body": "Hi {client_name},\n\nA reminder that invoice {invoice_id} for {currency} {amount} fell due on {due_date}.\n{pay_link}\nIf it has already been paid, thank you and please ignore this. Otherwise, could you let me know when to expect it?\n\nMany thanks,\n{sender_name}",
     "tips": ["Never write 'sorry to chase'. You are not doing anything wrong.", "One ask per email. Here it is 'when to expect it'.", "Keep the subject line factual so it is findable in their inbox later."]},
    {"slug": "payment-reminder-text-message", "step": "friendly", "offset": 3,
     "title": "Payment reminder text message (SMS) template",
     "h1": "Payment reminder by text message: when it works and what to say",
     "when": "Three or more days past due, for clients you normally text with, and only after at least one email. Never as the first contact.",
     "why": "A text is read within minutes; an email may sit for days. The trade-off is that it feels personal, so it must be short, specific, and easy to act on from a phone.",
     "subject": "(no subject; SMS)",
     "body": "Hi {client_name}, quick one: invoice {invoice_id} for {currency} {amount} was due {due_date}. Pay here: {pay_link} Let me know if there is any problem. Thanks, {sender_name}",
     "tips": ["Under 160 characters where possible; include the pay link.", "Only text clients who have texted you first about work.", "Follow it with the normal email ladder; the text is a nudge, not a record."]},
    {"slug": "invoice-60-days-overdue-demand-letter", "step": "escalate", "offset": 60,
     "title": "Invoice 60 days overdue: demand letter before collections or small claims",
     "h1": "60 days overdue: the letter before action",
     "when": "Sixty days past due, after the reminder ladder ran and a phone call went nowhere. This is a formal letter, sent by email and, ideally, by post.",
     "why": "A letter before action is the last step that costs nothing and the first one a court or collections agency will ask about. It states the debt, the deadline, and what happens next, without drama.",
     "subject": "Letter before action: invoice {invoice_id}, {currency} {total_due} outstanding",
     "body": "Dear {client_name},\n\nDespite reminders sent on the dates listed below, invoice {invoice_id} for {currency} {amount}, due on {due_date}, remains unpaid. With late fees under our agreed terms the balance is now {currency} {total_due}.\n\nUnless payment in full is received within 14 days of the date of this letter, I intend to pursue recovery, which may include a small-claims filing or referral to a collections agency, and to seek the additional costs that entails.\n\nIf you dispute this invoice, please set out the reasons in writing within the same 14 days.\n\nReminders sent: {steps_sent}\n\nYours sincerely,\n{sender_name}",
     "tips": ["List the dates of every reminder. A tool that logs sends gives you this for free.", "Fourteen days is the customary window in many jurisdictions; check yours.", "Only threaten what you will do. If small claims is not worth it for this amount, say 'collections' or say nothing."]},
    {"slug": "invoice-reminder-for-recurring-clients", "step": "courtesy", "offset": -3,
     "title": "Invoice reminder email for recurring (retainer) clients",
     "h1": "Reminding a retainer client without souring a good relationship",
     "when": "Before the due date, every month, as routine. Retainer clients are late by drift, not by intent; the pre-due note prevents the drift.",
     "why": "With a monthly client the relationship matters more than any single invoice, so the reminder is framed as routine housekeeping from the accounts side, not as a complaint from you.",
     "subject": "This month's invoice {invoice_id}, due {due_date}",
     "body": "Hi {client_name},\n\nRoutine note from the accounts side: this month's invoice {invoice_id} for {currency} {amount} is due on {due_date}.\n{pay_link}\nIf you would like to set up a standing payment so these stop landing in your inbox, reply and we will sort it out.\n\nThanks as always,\n{sender_name}",
     "tips": ["Offer autopay or a standing order in the email itself. It removes the problem for good.", "Send it from the accounts address even if you are the whole company; it keeps the personal relationship separate.", "If a retainer client slips twice, move them to payment in advance."]},
    {"slug": "invoice-reminder-email-subject-lines", "step": "firm", "offset": 7,
     "title": "Invoice reminder email subject lines that get opened",
     "h1": "Subject lines for payment reminders, by stage",
     "when": "Every reminder. The subject line decides whether the email is opened today or found during a quarterly inbox clean-up.",
     "why": "Good reminder subjects contain the invoice number and the amount (so the email is findable and specific) and change with the stage (so the escalation is visible before the email is opened).",
     "subject": "Overdue: invoice {invoice_id} ({days_overdue} days past due)",
     "body": "Before due:   Invoice {invoice_id} is due on {due_date}\nDay 1:        Invoice {invoice_id} was due on {due_date}\nDay 7:        Overdue: invoice {invoice_id} ({days_overdue} days past due)\nDay 14:       Final notice: invoice {invoice_id}, {currency} {total_due} now due\nDay 30+:      Letter before action: invoice {invoice_id}, {currency} {total_due} outstanding\n\nAvoid: 'Quick question', 'Following up', 'Checking in', anything without the invoice number.",
     "tips": ["Put the invoice number in every subject; it is what accounts-payable teams search for.", "Show the amount once the fee is applied; a number that went up gets attention.", "Do not use 'URGENT' or exclamation marks. Firm reads as calm."]},
...
JURISDICTIONS = [
    ...
    {"slug": "germany", "name": "Germany", "title": "Verzugszinsen: late payment interest on invoices in Germany",
     "summary": "In Germany, section 288 of the Civil Code (BGB) gives businesses a statutory right to default interest on late B2B invoices at 9 percentage points above the Bundesbank base rate, plus a flat €40 for recovery costs, and default begins automatically 30 days after the invoice is received.",
     "points": [
        "Statutory B2B default interest: 9 percentage points above the base rate (Basiszinssatz), which the Bundesbank resets on 1 January and 1 July. With a base rate of 1.52% from 1 July 2026, that is 10.52% a year. For consumers the margin is 5 points.",
        "Flat compensation of €40 per late invoice when the debtor is a business, claimable without proving any dunning costs, on top of interest.",
        "Default starts automatically 30 days after receipt of the invoice (or of the goods or service) if the debtor is a business; no reminder (Mahnung) is required, though sending one is still normal practice.",
        "A higher contractual rate is allowed if agreed and not grossly unfair; the statutory rate is the floor, not a cap.",
        "The formal route after reminders is the court dunning procedure (Mahnverfahren), which is inexpensive and mostly online."],
     "badcop": "Set <code>late_fee_pct</code> to the annual statutory rate divided by 12 (10.52% a year ≈ 0.88 per month) and <code>late_fee_flat = 40</code>. Have the final-notice template cite \"Verzugszinsen gemäß § 288 Abs. 2 BGB\" and the €40 pauschale under § 288 Abs. 5.",
     "sources": [("§ 288 BGB, Verzugszinsen und sonstiger Verzugsschaden (gesetze-im-internet.de)", "https://www.gesetze-im-internet.de/bgb/__288.html"),
                 ("Zahlungsverzug im Geschäftsverkehr: was gilt? Stand 2026 (IT-Recht Kanzlei)", "https://www.it-recht-kanzlei.de/zahlungsverzug-unternehmer.html"),
                 ("Verzugszinsen B2B berechnen 2026 (kostenlose-erechnung.de)", "https://kostenlose-erechnung.de/ratgeber/verzugszinsen-b2b-40-euro-pauschale/")]},
    {"slug": "ireland", "name": "Ireland", "title": "Late payment interest on invoices in Ireland",
     "summary": "Ireland implements the EU Late Payment Directive through the Late Payment in Commercial Transactions Regulations 2012: statutory interest at the ECB rate plus 8 percentage points, and automatic compensation starting at €40 per late invoice, without needing a reminder.",
     "points": [
        "Statutory interest from 1 July 2026: 10.4% per annum (ECB rate 2.40% plus 8%), applied daily at 10.4% ÷ 365. The rate resets every 1 January and 1 July.",
        "Compensation for recovery costs is automatic: €40 for debts under €1,000, with a rising scale for larger amounts set out in the Regulations' schedule.",
        "Applies to business-to-business and business-to-public-body transactions; public bodies are expected to pay within 30 days under the Prompt Payment code.",
        "The default payment period is 30 days unless otherwise agreed; longer B2B terms must not be grossly unfair.",
        "The right exists even if your contract says nothing about interest, but your own reasonable terms can replace it."],
     "badcop": "Use <code>late_fee_pct</code> of about 0.87 per month (10.4% a year) and <code>late_fee_flat = 40</code>. Cite \"statutory interest under the European Communities (Late Payment in Commercial Transactions) Regulations 2012\" in the final notice.",
     "sources": [("Late payment interest rate (Department of Enterprise, Tourism and Employment)", "https://enterprise.gov.ie/en/what-we-do/supports-for-smes/late-payments/late-payment-interest-rate-/"),
                 ("Late payment in commercial transactions (DETE)", "https://enterprise.gov.ie/en/what-we-do/supports-for-smes/late-payments/late-payment-in-commercial-transactions/"),
                 ("Late payment laws Ireland: statutory interest and business rights (Browne Legal)", "https://brownelegal.ie/late-payment-laws-ireland/")]},
    {"slug": "new-zealand", "name": "New Zealand", "title": "Charging interest on overdue invoices in New Zealand",
     "summary": "New Zealand has no statutory right to interest on a late invoice. You can charge it only if your written terms of trade say so and the customer accepted them before the work; a surprise charge added to an invoice is unenforceable and may breach the Fair Trading Act.",
     "points": [
     ...
     "badcop": "Set <code>late_fee_pct</code> to what your terms of trade say (1.0 to 1.5 is typical) and quote the clause in the final-notice template. If your terms are silent, leave the fee at 0 and fix the terms for future work.",
     "sources": [("Late payment fees in New Zealand: what businesses can legally charge (Sprintlaw NZ)", "https://sprintlaw.co.nz/articles/late-payment-fees-in-new-zealand-what-businesses-can-legally-charge/"),
                 ("Overdue invoice interest in NZ: can you charge it, and how much? (TradeFlow)", "https://tradeflow.org.nz/guides/overdue-invoice-interest-nz"),
                 ("How much can I charge for late fees in New Zealand? (Paidnice)", "https://www.paidnice.com/blog/late-fees-in-new-zealand")]},
]
...
<a class="card" href="late-fees/"><b>Late fees by country</b><span>UK, EU, US, Germany, Ireland, Australia, Canada, NZ</span></a>
